...
public class FUBAR {
  ...
  public static TileType[] generateMap(int boardSize) {
    if (boardSize < 6 || boardSize % 2 != 0)
      throw new RuntimeException("The board size must be even and at least 6.");
    ...
    Node<GridCoordinate> startNode =
        GridNode.findNode(board, 0, (int) (Math.random() * (boardSize - 2) + 1), boardSize);
    Node<GridCoordinate> endNode =
        GridNode.findNode(
            board,
            (int) Math.floor(boardSize / 2.0) - 2,
            (int) Math.floor(boardSize / 2.0) - 1,
            boardSize);
    ...
    startNode =
        GridNode.findNode(
            board, (int) (Math.random() * (boardSize - 2) + 1), boardSize - 1, boardSize);
    endNode =
        GridNode.findNode(
            board,
            (int) Math.floor(boardSize / 2.0) - 1,
    ...
    startNode =
        GridNode.findNode(
            board, boardSize - 1, (int) (Math.random() * (boardSize - 2) + 1), boardSize);
    endNode =
        GridNode.findNode(
            board,
            (int) Math.floor(boardSize / 2.0) + 1,
            (int) Math.floor(boardSize / 2.0),
            boardSize);
    path = AStar.findPath(board, startNode, endNode, excludePathNodes);
    if (path == null) return null;
    paths.add(path);
    ...
    for (int i = 0; i < paths.size(); i++) {
      ...
      final List<Node<GridCoordinate>> availableFirst =
          paths
              .get(i)
              .stream()
              .filter(node -> !((GridNode) node).hasDirection(firstDirection))
              .collect(Collectors.toList());
      final List<Node<GridCoordinate>> availableSecond =
          paths
              .get(secondIndex)
              .stream()
              .filter(node -> !((GridNode) node).hasDirection(secondDirection))
              .collect(Collectors.toList());
      ...
      for (int j = 0; j < maxIntersections; j++) {
        ...
        Node<GridCoordinate> firstNext =
            ((GridNode) firstElement).findNextNode(board, firstDirection);
        Node<GridCoordinate> secondNext =
            ((GridNode) secondElement).findNextNode(board, secondDirection);
    ...
    for (Node<GridCoordinate> node : board) {
      ...
      List<Node<GridCoordinate>> availableNodes =
    ...
    board[GridCoordinate.toIndex(boardSize / 2 - 2, boardSize / 2 - 1, boardSize)].addDirection(
        Direction.DOWN);
    board[GridCoordinate.toIndex(boardSize / 2, boardSize / 2 - 2, boardSize)].addDirection(
        Direction.RIGHT);
    board[GridCoordinate.toIndex(boardSize / 2 + 1, boardSize / 2, boardSize)].addDirection(
        Direction.UP);
    board[GridCoordinate.toIndex(boardSize / 2 - 1, boardSize / 2 + 1, boardSize)].addDirection(
        Direction.LEFT);
