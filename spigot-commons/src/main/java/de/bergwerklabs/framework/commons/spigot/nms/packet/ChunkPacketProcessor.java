...
public class ChunkPacketProcessor {
  ...
  public void process(ChunkletProcessor processor) {
    ...
    // The total size of a chunk is the number of blocks sent (depends on the number of sections)
    // multiplied by the
    // amount of bytes per block. This last figure can be calculated by adding together all the data
    // parts:
    //   For any block:
    ...
    size =
        BYTES_PER_NIBBLE_PART
                * ((NIBBLES_REQUIRED + skylightCount) * chunkSectionNumber + extraSectionNumber)
   ...
   *
   * <p>This is usually one for The Overworld, and zero for both The End and The Nether.
   *
   * @return Number of skylight byte segments.
   */
  protected int getSkylightCount() {
    // There's no sun/moon in the end or in the nether, so Minecraft doesn't sent any skylight
    // information
    // This optimization was added in 1.4.6. Note that ideally you should get this from the "f"
    // (skylight) field.
  ...
  private void translate(ChunkletProcessor processor) {
    ...
    if (hasContinous) {
      processor.processBiomeArray(
          new Location(world, chunkX << 4, 0, chunkZ << 4),
          data,
          startIndex + size - BIOME_ARRAY_LENGTH);
    }
  }
  ...
  public interface ChunkletProcessor {
     ...
     *
     * <p>This method will not be called if the chunk is missing biome information.
     *
  ...
  public static class ChunkOffsets {
    ...
    private ChunkOffsets(
        int blockIdOffset, int dataOffset, int lightOffset, int skylightOffset, int extraOffset) {
     ...
     *
     * <p>This will be 4096 bytes in lenght, one byte for each block in the 16x16x16 chunklet.
     *
     ...
     *
     * <p>This will be 2048 bytes in lenght, one nibblet for each block in the 16x16x16 chunklet.
     *
     ...
     *
     * <p>This will be 2048 bytes in lenght if the skylight data exists (see {@link
     * #hasSkylightOffset()}), no bytes if not.
     *
     ...
     *
     * <p>This will be 2048 bytes in lenght if the extra data exists, no bytes if not.
     *
