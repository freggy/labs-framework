 ...
 *
 * <p>Class for creating ItemStacks
 *
...
public class ItemStackUtil {
  ...
  public static ItemStack createItemStackFromJson(JsonObject json) {
    ...
    if (json.has("material")) builder.setType(Material.valueOf(json.get("material").getAsString()));

    if (json.has("name")) builder.setName(json.get("name").getAsString());

    if (json.has("amount")) builder.setAmount(json.get("amount").getAsInt());

    if (json.has("damage")) builder.setDamage(json.get("damage").getAsShort());

    if (json.has("lore"))
      builder.setLore(JsonUtil.jsonArrayToStringList(json.get("lore").getAsJsonArray()));

    if (json.has("data")) builder.setData(json.get("data").getAsByte());

    if (json.has("enchantments")) {
      EnchantmentWrapper.enchantmentsFromJson(json.get("enchantments").getAsJsonArray())
          .forEach(
              enchantment -> {
  ...
  public static ItemStack potionAsItemStackFromJson(JsonObject json) {
    ...
    if (itemStackForMeta.hasItemMeta()) item.setItemMeta(itemStackForMeta.getItemMeta());
    return item;
  }
  ...
  public static Potion potionFromJson(JsonObject json) {
    ...
    if (json.has("splash")) builder.setSplash(json.get("splash").getAsBoolean());

    if (json.has("level")) builder.setLvl(json.get("level").getAsInt());
