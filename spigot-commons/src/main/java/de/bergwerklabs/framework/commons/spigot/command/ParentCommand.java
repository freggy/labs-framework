 ...
 * <p>
 * Class that represents a parent command.
 *
 * Example:
 * <p>
 * Lets look at the party creation command: {@code /party create}
 * <p>
 * In this example the parent command would be /party. The sub command is create.
 * A sub command can in some cases also be a parent command.
 *
...
public abstract class ParentCommand implements CommandExecutor {
     ...
     * @param childCommands child commands of this command.
     */
    public ParentCommand(String command, CommandExecutor executor, Collection<ChildCommand> childCommands) {
        if (childCommands.size() == 0) throw new IllegalArgumentException("There cannot be 0 child commands, at least 1 has to be present");
