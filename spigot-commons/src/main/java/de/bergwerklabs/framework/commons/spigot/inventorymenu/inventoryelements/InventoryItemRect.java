 ...
 *
 * <p>Rectangle class for InventoryMenus
 *
...
public class InventoryItemRect extends InventoryMenuElement {
   ...
   */
  public InventoryItemRect(
      Integer topLeftSlot,
      Integer bottomRightSlot,
      InventoryItem borderItem,
      InventoryItem fillItem) {
  ...
  public void render(Inventory inventory) {
    ...
    if (this.full) this.renderFull(inventory);
    else if (this.borderOnly) this.renderBorder(inventory);
    else this.renderOuterAndInner(inventory);
  }
  ...
  private void renderOuterAndInner(Inventory inventory) {
    ...
    this.renderInnerSpace(
        ((this.topLeftSlot / ROW_LENGTH) * ROW_LENGTH) + ROW_LENGTH + 1, inventory);
  }
  ...
  private void renderFull(Inventory inventory) {
    ...
    for (int row = 0; row < rows; row++) {
      ...
      InventoryItemRowSpan span =
          new InventoryItemRowSpan(currentRow, currentRow + 8, this.fillItem);
  ...
  private void placeOuterItems(Inventory inventory) {
    ...
    // The number before the decimal point represents the slot, the number after the row specific
    // slot.
