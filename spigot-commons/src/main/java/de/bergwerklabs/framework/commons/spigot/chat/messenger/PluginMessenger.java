...
public class PluginMessenger {
  ...
  public PluginMessenger(String name) {
    this.prefix =
        "§6>> §e" + name + " §6❘§7 "; // TODO: use utf8 symbol instead of '|', make configurable  in
    // framework config.
  }
   ...
   * This class provides the {@link Player} and the prefix that should be used when calling {@link
   * Player#sendMessage(String)}. The recommended way is to invoke {@link
   * PluginMessenger#message(String, Player)} within {@link Consumer#accept(Object)}.
   *
  ...
  /**
   * Invokes {@link Consumer#accept(Object)} with an instance of the {@link ConsumerInfo} class on
   * every player in the collection.
   *
   * @param func {@link Consumer} used for providing more advanced behavior.
   * @param players {@link Collection} of players.
   */
