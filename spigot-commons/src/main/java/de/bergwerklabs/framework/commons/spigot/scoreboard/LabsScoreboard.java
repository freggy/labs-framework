...
public class LabsScoreboard implements Versionable, Identifiable, Cloneable {
  ...
  /**
   * @param sidebarName Title that the side bar should have. NOTE: Title can only consist of 32
   *     chars at max.
  ...
  public Map<String, Row> getRowsByContent() {
    return this.rowsByContent;
  }

  public Map<UUID, Row> getPlayerSpecificRows() {
    return playerSpecificRows;
  }
   ...
   *
   * @param index Value where the row should be placed. e.g. index = 1 would be the top row of the
   *     scoreboard
