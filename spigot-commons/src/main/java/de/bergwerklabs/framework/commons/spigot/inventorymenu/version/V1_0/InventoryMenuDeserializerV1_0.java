 ...
 *
 * <p>Class for deserializing scheme version 1.0
 *
...
public class InventoryMenuDeserializerV1_0 implements Deserializer<InventoryMenu> {
  ...
  public InventoryMenu deserialize(JsonObject json) {
    ...
    if (json.has("on-preprocess"))
      this.onPreprocessMethod =
          OnPreprocessMethod.fromJson(json.get("on-preprocess").getAsJsonObject(), controller);
    ...
    if (json.has("item-row-spans"))
      json.get("item-row-spans")
          .getAsJsonArray()
          .iterator()
          .forEachRemaining(
              span ->
                  rowSpans.add(
                      this.createItemRowSpan(span.getAsJsonObject(), controller, inventory)));

    if (json.has("item-column-spans"))
      json.get("item-column-spans")
          .getAsJsonArray()
          .iterator()
          .forEachRemaining(
              span ->
                  columnSpans.add(
                      this.createItemColumnSpan(span.getAsJsonObject(), controller, inventory)));
    ...
    if (json.has("item-rects"))
      json.get("item-rects")
          ...
          .forEachRemaining(
              rect ->
                  rects.add(this.createItemRect(rect.getAsJsonObject(), controller, inventory)));

    if (json.has("items"))
      JsonUtil.jsonArrayToJsonObjectList(json.get("items").getAsJsonArray())
          .forEach(
              item ->
                  items.add(this.createInventoryItemFromJson(item, controller, inventory, false)));
   ...
   */
  private InventoryItemRect createItemRect(
      JsonObject json, LabsController controller, Inventory inventory) {
    InventoryItem borderItem = null, fillItem = null;

    if (json.has("border-item"))
      borderItem =
          createInventoryItemFromJson(
              json.get("border-item").getAsJsonObject(), controller, inventory, true);

    if (json.has("fill-item"))
      fillItem =
          createInventoryItemFromJson(
              json.get("fill-item").getAsJsonObject(), controller, inventory, true);

    return new InventoryItemRect(
        json.get("top-left-slot").getAsInt(),
        json.get("bottom-right-slot").getAsInt(),
        borderItem,
        fillItem);
  }
   ...
   */
  private InventoryItemRowSpan createItemRowSpan(
      JsonObject json, LabsController controller, Inventory inventory) {
    return new InventoryItemRowSpan(
        json.get("start-slot").getAsInt(),
        json.get("end-slot").getAsInt(),
        this.createInventoryItemFromJson(json, controller, inventory, true));
  }
   ...
   */
  private InventoryItemColumnSpan createItemColumnSpan(
      JsonObject json, LabsController controller, Inventory inventory) {
    return new InventoryItemColumnSpan(
        json.get("start-slot").getAsInt(),
        json.get("end-slot").getAsInt(),
        this.createInventoryItemFromJson(json, controller, inventory, true));
  }
   ...
   */
  private InventoryItem createInventoryItemFromJson(
      JsonObject item, LabsController controller, Inventory inventory, boolean isChild) {
    ...
    if (item.has("update")) update = UpdateMethod.fromJson(item.get("update"), controller);
    ...
    if (item.has("item"))
      ...
      itemStack =
          EnchantedBookWrapper.fromJson(item.get("enchanted-book").getAsJsonObject()).getBook();
    }

    // slot can be optional
    if (item.has("slot")) {
      return new InventoryItem(
          item.get("slot").getAsInt(), itemStack, onClick, update, inventory, isChild);
    } else return new InventoryItem(null, itemStack, onClick, update, inventory, isChild);
  }
}
