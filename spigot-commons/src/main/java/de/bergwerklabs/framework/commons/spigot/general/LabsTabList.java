 ...
 *
 * <p>Contains functionality for creating header and footer for the tab list.
 *
...
public class LabsTabList {
  ...
  public void broadcast() {
    Bukkit.getServer()
        .getOnlinePlayers()
        .forEach(player -> this.sendHeaderAndFooter(this.header, this.footer, player));
  }
  ...
  private void sendHeaderAndFooter(String header, String footer, Player player) {
    ...
    try {
      Object packet =
          LabsReflection.getNmsClass("PacketPlayOutPlayerListHeaderFooter")
              .getConstructor(LabsReflection.getNmsClass("IChatBaseComponent"))
              .newInstance(NmsUtil.getIChatBaseComponent(header));

      LabsReflection.setFieldValue(
          LabsReflection.getField(packet.getClass(), "b"),
          packet,
          NmsUtil.getIChatBaseComponent(footer));
