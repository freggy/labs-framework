 ...
 *
 * <p>Deserializer for v1.0 LabsScoreboards
 *
...
public class LabsScoreboardDeserializerV1_0 implements Deserializer<LabsScoreboard> {
  ...
  public LabsScoreboard deserialize(JsonObject json) {
    ...
    if (json.has("class")) controller = VersionedJsonReflectionUtil.controllerFromJson(json);

    LabsScoreboard scoreboard =
        new LabsScoreboard(json.get("title").getAsString(), json.get("id").getAsString());

    JsonUtil.jsonArrayToJsonObjectList(json.get("rows").getAsJsonArray())
        .stream()
        .forEach(
            jsonRow -> {
   ...
   */
  private Row createRowFromJson(
      JsonObject jsonRow, LabsScoreboard scoreboard, LabsController controller) {
    ...
    if (jsonRow.has("content")) text = jsonRow.get("content").getAsString();
    else {
      blank +=
          ChatColor.RED.toString(); // duplicate lines will not get drawn, so add char everytime.
      text = blank;
    }
