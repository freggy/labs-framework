...
public class PacketBuilder<T extends Packet> {
  ...
  protected void assignPacketClass(
      MinecraftVersion version, String packageString, String className) {
    try {
      this.packet =
          (T)
              Class.forName(
                      "de.bergwerklabs.framework.commons.spigot.nms.packet.serverside."
                          + packageString
                          + "."
