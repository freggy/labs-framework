...
public class NbtUtil {
  ...
  /**
   * Converts a {@link com.sk89q.worldedit.Vector} to NBT. The {@link CompoundTag} is structured as
   * follows:
   *
   * <pre>
   *     <code>
  ...
  public static CompoundTag vectorToNbt(String name, Double x, Double y, Double z) {
    return newCompoundTag(
        name, new DoubleTag("x", x), new DoubleTag("y", y), new DoubleTag("z", z));
  }

  /**
   * Converts a {@link CompoundTag} to a {@link Vector}. Structure of the {@link CompoundTag}:
   *
   * <pre>
   *     <code>
   *        TAG_Compound("name"): 3 entries {
  ...
  public static Vector vectorFromNbt(CompoundTag tag) {
    return new Vector(
        new Double(tag.getValue().get("x").getValue().toString()),
